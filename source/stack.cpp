...
int StackCtor(StackString_t* stk, int capacity, int line, const char* stackname, const char* funcname) {
    ...
    #ifdef CANARY
        stk->data = (StackElement_t* ) calloc((size_t) capacity + 2, sizeof(StackElement_t));
    #else
        stk->data = (StackElement_t* ) calloc((size_t) capacity,     sizeof(StackElement_t));
    #endif
...
int StackRealloc(StackString_t* stk, int line) {
    ...
    #ifdef CANARY
    StackElement_t* stkreal = (StackElement_t* ) realloc(stk->data, (2 * (size_t) stk->capacity + 2) * sizeof(stk->data[0]));
    #else
    StackElement_t* stkreal = (StackElement_t* ) realloc(stk->data, (2 * (size_t) stk->capacity) * sizeof(stk->data[0]));
    #endif
...
// int StackPrint(StackString_t* stk, int line) {
//     int err = 0;
